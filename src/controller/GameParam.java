package controller;

public final class GameParam {
	
	// Mode
	public final static boolean TEST_MODE = true;
	public final static boolean IMMUNITY_MODE = false;
	public final static boolean INFINITE_LIFE_MODE = true;
	
	// Timer
	public final static int RESET_POINT = 60 * 60 * 20;
	
	// Time Stop
	public final static int TIME_STOP_LENGTH = 60 * 3;
	
	// Status
	public final static int LIFE_LEFT = 3;
	public final static int NO_LIFE_LEFT = 2;
	public final static int RESPAWN_IMMUNITY_TIME = 60 * 2;
	public final static int RESPAWNING_TIME = 30;
	
	// Bomb
	public final static int INIT_BOMB_CAPACITY = 3;
	public final static int BOMB_DAMAGE = 200;
	
	// Life
	public final static int DEFAULT_LIFE = 1;
	
	// Player Speed
	public final static double DEFAULT_SPEED = 4;
	public final static double SLOW_SPEED = 1.8;
	
	// Player Bullet

}
